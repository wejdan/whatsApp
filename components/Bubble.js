...
import md5 from 'md5';
import {unstarMessage} from '../store/messagesSlice';
...
const Bubble = ({
  ...
  const isStarred = useSelector(
    state => state.messages.starredMessages[starId],
  );
  ...
  const menuName = useRef(uuid.v4()).current;
  if (type == 'system') {
  ...
  return (
    <View style={container}>
      <Container
        ...
        onLongPress={() => {
          if (!isStar) {
            menuRef.current.props.ctx.menuActions.openMenu(menuName);
          } else {
            menuRef.current.props.ctx.menuActions.openMenu(
              `starredMenu-${messageId}`,
            );
          }
        }}>
        <View style={bubbleStyle}>
          ...
          {isStar ? (
            ...
            <Menu name={menuName} ref={menuRef}>
              <MenuTrigger />
              <MenuOptions>
                <MenuItem
